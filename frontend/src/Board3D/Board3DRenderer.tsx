...
import {
  ...
  type BoardProjectedPosition as ProjectedBoardPosition,
  type BoardProjection,
} from "../boardProjection";
import { TargetingOverlay } from "../targetingOverlay";
import { cameraBoardProjectionAdapter } from "./cameraBoardProjection";
...
export function Board3DRenderer({
  ...
  const [projectedHitTargets, setProjectedHitTargets] = useState<BoardProjection>(() =>
    createBoardProjection([]),
  );
      ...
      >
        ...
        <div className="board-3d-hit-layer" aria-label="3D board controls">
          <TargetingOverlay
            ...
            projection={projectedHitTargets}
          />
          {tileInteractions.map((interaction) => (
            <Board3DHitTarget
              key={coordKey(interaction.coord)}
              interaction={interaction}
              projectedPosition={projectedHitTargets.positionAt(interaction.coord)}
              readOnly={readOnly}
              disabled={disabled}
              onClick={handleTileClick}
...
function ProjectedHitTargetSync({
  ...
  onPositionsChange: (positions: BoardProjection) => void;
}) {
  const { camera, size } = useThree();
  const previousPositionsRef = useRef<BoardProjection>(createBoardProjection([]));

  useFrame(() => {
    camera.updateMatrixWorld();
    const nextPositions = cameraBoardProjectionAdapter.project(
      tileInteractions.map((interaction) => interaction.coord),
      { camera, viewport: size },
      previousPositionsRef.current,
    );

    if (nextPositions !== previousPositionsRef.current) {
